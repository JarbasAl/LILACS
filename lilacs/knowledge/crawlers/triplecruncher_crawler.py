from lilacs.knowledge.data_sources import LILACSKnowledge
from lilacs.reasoning import LILACSTextAnalyzer
from lilacs.knowledge.crawlers import DummyCrawler


class TripleCruncher(DummyCrawler):
...
class RelationshipCruncher(DummyCrawler):
    ...
    def execute_action(self, connections):
        ...
        if label.lower() != self.current_node.name.lower():
            #print("dbpedia label does not match node name, synonym proposal")
            print("** triple:", (label, "is", self.current_node.name))
        description = node_data["description"]
...
if __name__ == "__main__":
    c = TripleCruncher(threaded=False)
    c.start_crawling("scientist")
    print(c.crawl_list)
    print(c.total_steps)
