...
def is_passive(doc):
    ...
    tagged = doc._.brown_tags
    tags = map(lambda tup: tup[1], tagged)
    after_to_be = list(
        dropwhile(lambda tag: not tag.startswith("BE"), tags))
    nongerund = lambda tag: tag.startswith("V") and \
...
def get_subjects_of_verb(verb):
    ...
    SUBJ_DEPS = {'agent', 'csubj', 'csubjpass', 'expl', 'nsubj',
                 'nsubjpass'}
...
def past_tense(doc):
    return change_tense(doc, "past")


def present_tense(doc):
    return change_tense(doc, "present")


def future_tense(doc):
    return change_tense(doc, "future")
